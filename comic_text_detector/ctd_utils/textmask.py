...
def get_topk_masklist(im_grey, pred_mask):
    ...
    for ii, color in enumerate(topk_color):
        ...
        threshed = np.uint8(cv2.inRange(im_grey, c_bottom, c_top))
...
def refine_mask(img: np.ndarray, pred_mask: np.ndarray, blk_list: list[Quadrilateral], refine_mode: int | None = REFINEMASK_INPAINT) -> np.ndarray:
